...
export function getDrivingThunk(origin: string, destination: string) {
    return async (dispatch: ThunkDispatch) => {
        ...
        const res = await fetch(`https://cors-anywhere.herokuapp.com/https://maps.googleapis.com/maps/api/directions/json?origin=${origin}&destination=${destination}&key=${process.env.REACT_APP_GOOGLE_KEY}`);
